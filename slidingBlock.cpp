 ...
 * return:  indicates success/failure
 */
bool validDirection(Snapshot ss, int rows, int cols, int pieceIndex, string dir){
 ...
 */
vector<Snapshot> getPossibleMoves(set<string>& seen, Snapshot currSS, int rows, int cols, int pieceIndex){
...
Snapshot BFS(Grid grid){
  ...
  while (!frontier.empty()){
    ...
    for(int pieceIndex = 0; pieceIndex < currSS.getPieces().size(); pieceIndex++){

      // get an ordered list of valid moves for this piece
      vector<Snapshot> validMoves = getPossibleMoves(seen, currSS, rows, cols, pieceIndex);
...
int main(int argc, char** argv) {
  ...
  if (checkSuccess(rows, columns, solution.getPieceLayout())){
    cout << "This puzzle is solvable in " << solution.getTotalAmount() << " steps" << endl;
